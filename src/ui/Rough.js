/* <div
        id='controls'
        className='self-start flex items-start bg-red-700 justify-stretch gap-2 mx-2 '
      >
        <Button type='cancel' />
...
/* acc.push({
        date: dateFormatter(call.created_at),
        calls: [call],
      }); */

/* let newArr = [];

const dateWiseUniqueCalls = dateWiseCalls.reduce((acc, dateCallObj) => {
  for (let i = 0; i < dateCallObj.calls.length; i++) {
    const callI = dateCallObj.calls.at(i);
    callI.noOfCalls = 1;
    for (let j = i + 1; j < dateCallObj.calls.length; j++) {
      const callJ = dateCallObj.calls.at(j);
       if (!callI.from && !callJ.from) {
          if (new Date(callI.created_at) > new Date(callJ.created_at)) {
            callI.created_at = callJ.created_at;
          }
          callI.noOfCalls++;
          dateCallObj.calls.pop(callJ);
        }
      if (callI.from === callJ.from) {
        newArr = [...newArr, callI];
      }
  ...
  acc.push({
    ...dateCallObj,
    calls: dateCallObj.calls,
  });
  return acc;
}, []);
 */

/* for (const [key, value] of dateCallsMap2) {
    // dateWiseCalls.key
    const callArr = dateCallsMap.get(key);
    let noOfCalls = 0;
    
    for (let i = 0; i < callArr.length; i++) {
      const callI = callArr[i];
      noOfCalls = 1;
      if (!callI.from) callI.from = 'Unknown';
      
      for (let j = i + 1; j < callArr.length; j++) {
        const callJ = callArr[j];
        if (!callJ.from) callJ.from = 'Unknown';
...
/* if (!fromNoOfCalls[`${callI.from}`]) {
      fromNoOfCalls[`${callI.from}`] = 1;
    } else {
      fromNoOfCalls[`${callI.from}`] = fromNoOfCalls[`${callI.from}`]++;
    } */

/* const callArr = state.calls;
let fromNoOfCalls = {};
...
for (let i = 0; i < callArr.length; i++) {
  ...
  for (let j = i + 1; j < callArr.length; j++) {
    ...
    if (
      format(callI.created_at, 'MMMM, dd yyyy') ===
      format(callJ.created_at, 'MMMM, dd yyyy')
    ) {
      if (callI.from === callJ.from) {
        noOfCalls++;
      }
  ...
  if (
    !fromNoOfCalls[`${format(callI.created_at, 'MMMM, dd yyyy')}:${callI.from}`]
  )
    fromNoOfCalls[
      `${format(callI.created_at, 'MMMM, dd yyyy')}:${callI.from}`
    ] = noOfCalls;
} */

/* export function areAllCallsArchived(callsArr) {
  let areAllCallsArchived = callsArr[0].is_archived;

  callsArr.forEach((call) => {
    if (!call.is_archived) {
      areAllCallsArchived = call.is_archived;
    }
  });

  return callsArr.some((call) => call.is_archived !== false);
  return areAllCallsArchived;
} */
...
/* <li>
      ...
      <li className='mr-1'>
        <button onClick={() => dispatch({ type: 'call/load' })}>
